...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        render_mode = os.environ.get("SIMLINGO_RENDER_MODE", "offscreen")
        ...
        cmd1 = " ".join(shlex.quote(part) for part in cmd_parts)
        carla_log_path = os.environ.get("SIMLINGO_CARLA_LOG")
        self.server_log = None
        if carla_log_path:
            os.makedirs(os.path.dirname(carla_log_path), exist_ok=True)
            self.server_log = open(carla_log_path, "a", buffering=1)
        self.server = subprocess.Popen(
            cmd1,
            shell=True,
            preexec_fn=os.setsid,
            stdout=self.server_log if self.server_log else None,
            stderr=subprocess.STDOUT if self.server_log else None,
        )
