...
class LingoAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.use_model_route_as = self.config.eval_route_as == -1

        self.last_command = -1
        self.last_command_tmp = -1
        self.user_command = None
        user_flag = os.environ.get("SIMLINGO_USER_FLAG")
        self.user_flag = int(user_flag) if user_flag and user_flag.lstrip("-").isdigit() else None
        self.running = True
        self.custom_prompt = os.environ.get("SIMLINGO_CUSTOM_PROMPT") or None
        ...
        if self.use_model_route_as:
            self.config.eval_route_as = getattr(self.model, "route_as", self.cfg.data_module.base_dataset.route_as)
        print(f"SIMLINGO_ROUTE_CONDITIONING={self.config.eval_route_as}", flush=True)
        if self.custom_prompt is not None:
            print(f"SIMLINGO_CUSTOM_PROMPT={self.custom_prompt}", flush=True)
        try:
            self.profile_every = max(0, int(os.environ.get("SIMLINGO_PROFILE_EVERY", "40")))
        except ValueError:
        ...
        print(
            "SIMLINGO_NATIVE_MODE: full model inference every CARLA tick, "
            "language kept enabled by checkpoint config, no demo safety post-processing.",
            flush=True,
        )
    ...
    def tick(self, input_data):
        ...
        if 'internvl2' in self.cfg.model.vision_model.variant.lower():
            T, C, H, W = rgbs.shape
            transform = getattr(self, "image_transform", None)
        ...
        if getattr(self, "conv_module", None) is None:
    ...
    def _draw_native_waypoints(self, pred_route, pred_speed_wps):
        if not getattr(self, "draw_waypoints", False):
            return
        try:
            ...
            def ego_to_world(point):
                x = float(point[0])
                y = float(point[1])
                return carla.Location(
                    x=origin.x + forward.x * x + right.x * y,
                    y=origin.y + forward.y * x + right.y * y,
                    z=origin.z + 0.35,
                )
            ...
            def draw_points(points, color, size):
                ...
                for point in points[:32]:
                    if not np.isfinite(point).all():
                        continue
                    if np.linalg.norm(point[:2]) > 80.0:
                        continue
            ...
            draw_points(self.target_points, carla.Color(0, 70, 255), 0.10)
            draw_points(pred_route[0], carla.Color(255, 0, 0), 0.075)
            draw_points(pred_speed_wps[0], carla.Color(0, 255, 0), 0.06)
        except Exception as exc:
            if self.profile_every and self.step % self.profile_every == 0:
                print(f"SIMLINGO_DRAW_WAYPOINTS skipped: {exc}", flush=True)
    ...
    def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        # Need to run this every step for GPS filtering and route-planner updates.
        ...
        if pred_route is None or pred_speed_wps is None:
            control = carla.VehicleControl(steer=0.0, throttle=0.0, brake=1.0)
            self.control = control
            return control

        self._draw_native_waypoints(pred_route, pred_speed_wps)
        ...
        if self.profile_every and self.step > 0 and self.step % self.profile_every == 0:
            control_debug = getattr(self, "last_control_debug", {})
            desired_dbg = float(control_debug.get("desired_speed", float("nan")))
            throttle_dbg = float(control_debug.get("throttle", float("nan")))
            steer_dbg = float(control_debug.get("steer", float("nan")))
            brake_dbg = int(bool(control_debug.get("brake", False)))
            language_dbg = ""
            if language:
                language_dbg = str(language[0]).replace("\n", " ").strip()
                language_dbg = language_dbg[:220]
            print(
                "SIMLINGO_PROFILE "
                f"step={self.step} tick={tick_elapsed:.3f}s "
                f"model={model_elapsed:.3f}s control={control_elapsed:.4f}s "
                f"speed={current_speed:.2f}m/s desired={desired_dbg:.2f}m/s "
                f"model_throttle={throttle_dbg:.2f} model_brake={brake_dbg} "
                f"cmd_throttle={float(throttle):.2f} cmd_brake={int(bool(brake))} "
                f"steer={steer_dbg:.2f}",
                flush=True,
            )
            dreamer_dbg = getattr(self, "last_dreamer_guard_info", {})
            if dreamer_dbg:
                print(
                    "SIMLINGO_DREAMER_PROFILE "
                    f"step={self.step} mode={dreamer_dbg.get('mode')} "
                    f"candidate={dreamer_dbg.get('candidate_index')} "
                    f"applied={int(bool(dreamer_dbg.get('applied')))} "
                    f"risk={float(dreamer_dbg.get('base_risk', float('nan'))):.3f}->"
            ...
            if language_dbg:
                print(f"SIMLINGO_LANGUAGE step={self.step} text={language_dbg}", flush=True)
    ...
    def control_pid(self, route_waypoints, velocity, speed_waypoints):
        ...
        desired_speed_safe = max(float(desired_speed), 1e-6)
        brake = ((desired_speed < self.config.brake_speed) or ((speed / desired_speed_safe) > self.config.brake_ratio))
    ...
    def destroy(self, results=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        if hasattr(self, "config"):
            del self.config
        data_module_encoder = getattr(self.cfg.data_module, "encoder", None)
        if data_module_encoder == 'llavanext' and hasattr(self, "processor"):
            del self.processor
