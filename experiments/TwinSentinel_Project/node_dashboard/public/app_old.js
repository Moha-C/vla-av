...
const clearBaselineBtn = document.getElementById("clearBaseline");

const HISTORY_KEY = "vanetBaselineHistory";
const HISTORY_ATTACKED_KEY = "vanetAttackedHistory";
const SELECTED_METRIC_KEY = "vanetSelectedMetric";

const state = {
  liveHistory: [],
  baselineHistory: loadStoredHistory(HISTORY_KEY),
...
function updateStatusLine() {
  ...
  baselineStatusEl.textContent = `Baseline: ${baselineCount ? `loaded (${baselineCount} points)` : "not loaded"} | Attacked: ${attackedCount ? `saved (${attackedCount} points)` : "not saved"} | Live points: ${liveCount}`;
}
...
function saveBaselineFromLive() {
  state.baselineHistory = [...state.liveHistory];
  saveStoredHistory(HISTORY_KEY, state.baselineHistory);
  updateStatusLine();
  applyAllViews();
}
...
captureBaselineBtn.addEventListener("click", () => {
  saveBaselineFromLive();
  captureBaselineBtn.textContent = "Baseline captured";
});

captureAttackedBtn.addEventListener("click", () => {
  saveAttackedFromLive();
  captureAttackedBtn.textContent = "Attacked captured";
});

clearBaselineBtn.addEventListener("click", () => {
  clearBaseline();
  clearBaselineBtn.textContent = "Baseline cleared";
});
...
(async () => {
  try {
    ...
    if (state.baselineHistory.length) {
      baselineStatusEl.textContent = `Baseline restored from browser storage (${state.baselineHistory.length} points)`;
    }
    if (state.attackedHistory.length) {
      captureAttackedBtn.textContent = `Attacked saved (${state.attackedHistory.length})`;
    }
    applyAllViews();
  } catch (error) {
