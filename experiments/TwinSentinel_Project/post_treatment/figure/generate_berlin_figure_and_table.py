...
def main():
    ...
    if n_seeds == 0:
        print("Error: No Berlin baseline seed files found.")
        return
        
    print(f"Loaded {n_seeds} Berlin baselines.")
    ...
    print("\n--- Baseline Features Stats (Berlin) ---")
    ...
    for att, display_name in attack_names.items():
        ...
        for s in range(1, n_seeds + 1):
            run_file = f"run_berlin_{att}_L3_seed_{s}.json"
    ...
    ax1.set_title("Comparative ROC Curves (Berlin)", fontsize=12, fontweight="bold")
    ...
    ax2.set_title("Detection Metrics at Significance Thresholds (Berlin)", fontsize=12, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "berlin_roc_pr.png"), dpi=300)
    plt.savefig(os.path.join(OUTPUT_DIR, "berlin_roc_pr.pdf"), format="pdf", dpi=300)
    print("\n✓ Saved Figure 9 curves to berlin_roc_pr.png & berlin_roc_pr.pdf")
    
    # Save the detailed table rows
    with open(os.path.join(TABLE_OUTPUT_DIR, "berlin_detailed_table_results.json"), "w") as f:
