...
def main():
    ...
    if n_seeds == 0:
        print("Error: No Luxembourg baseline seed files found.")
        return
        
    print(f"Loaded {n_seeds} Luxembourg baselines.")
    ...
    print("\n--- Baseline Features Stats (Luxembourg) ---")
    print(f"KPI Mean vector S: {mu_base}")
    ...
    for att, display_name in attack_names.items():
        ...
        for s in range(1, n_seeds + 1):
            run_file = f"run_lux_{att}_L3_seed_{s}.json"
    ...
    ax1.set_title("Comparative ROC Curves (Luxembourg)", fontsize=12, fontweight="bold")
    ...
    ax2.set_title("Detection Metrics at Significance Thresholds (Luxembourg)", fontsize=12, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "luxembourg_roc_pr.png"), dpi=300)
    plt.savefig(os.path.join(OUTPUT_DIR, "luxembourg_roc_pr.pdf"), format="pdf", dpi=300)
    print("\n✓ Saved Figure 9 curves to luxembourg_roc_pr.png & luxembourg_roc_pr.pdf")
    
    with open(os.path.join(TABLE_OUTPUT_DIR, "luxembourg_detailed_table_results.json"), "w") as f:
