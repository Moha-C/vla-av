...
def main():
    print("=" * 60)
    print("      Luxembourg VANET simulation campaign analysis      ")
    print("=" * 60)
    
    # 1. Load Luxembourg baselines
    baseline_files = [f for f in os.listdir(BASELINES_DIR) if f.startswith("baseline_lux_seed_") and f.endswith(".json")]
    n_seeds = len(baseline_files)
    if n_seeds == 0:
        print("No Luxembourg baseline files found in baselines/ directory. Run campaign script first.")
        return
        
    print(f"Loading {n_seeds} Luxembourg baseline runs...")
    
    seeds = sorted([int(f.replace("baseline_lux_seed_", "").replace(".json", "")) for f in baseline_files])
    fit_seeds = seeds[:10]
    ...
    fit_features = []
    for s in fit_seeds:
        with open(os.path.join(BASELINES_DIR, f"baseline_lux_seed_{s}.json"), "r") as f:
            hist = json.load(f)
    ...
    for att in attacks:
        ...
        for lvl in ["L1", "L2", "L3"]:
            ...
            for s in range(1, n_seeds + 1):
                run_file = f"run_lux_{att}_{lvl}_seed_{s}.json"
                ...
                tp = fp = tn = fn = 0
                for w in range(30):
                    pred = 1 if distances[w] > 0.5 * (tau_95 + tau_99) else 0  # calibrated threshold
                    pred_95 = 1 if distances[w] > tau_95 else 0
                    if y_true[w] == 1:
                        if pred_95 == 1: tp += 1
                        else: fn += 1
                    else:
    ...
    print("\n" + "=" * 150)
    print("TABLE V: DETECTION PERFORMANCE PER ATTACK ON THE LUXEMBOURG NETWORK")
    print("=" * 150)
    ...
    for att in attacks:
        ...
        table_v_rows.append([
            attack_names[att], f"{m_tpr:.1f}%", f"{m_fpr:.1f}%", f"{m_prec:.1f}%", f"{m_f1:.1f}%", f"{m_roc:.3f}", f"{m_pr:.3f}", f"{m_lat:.1f}s", top_kpis_str
        ])
    ...
    if avg_tpr:
        ...
        table_v_rows.append([
            "Mean", f"{np.mean(avg_tpr):.1f}%", f"{np.mean(avg_fpr):.1f}%", f"{np.mean(avg_prec):.1f}%", f"{np.mean(avg_f1):.1f}%", f"{np.mean(avg_roc):.3f}", f"{np.mean(avg_pr):.3f}", f"{np.mean(avg_lat):.1f}s", "―"
        ])
    print("=" * 60)
    
    # Holm-Bonferroni correction printout
    print("\n" + "=" * 90)
    print("HOLM-BONFERRONI STATISTICAL SIGNIFICANCE (LUXEMBOURG - ALPHA = 0.05)")
    print("=" * 90)
    ...
    for att, u_val, p_val, adj_p, sig in monotonized_p_values:
        ...
        hb_rows.append([
            attack_names[att], f"{u_val:.1f}", raw_p_str, adj_p_str, "YES" if adj_p < 0.05 else "NO"
        ])
    print("=" * 90)
    
    summary_path = "/home/mehdi/VANET_Project/Docker_files/post_treatment/table/luxembourg_campaign_analysis_summary.json"
    summary_data = {
    ...
    # 4. Generate Luxembourg Figure 10 (Frontier Plot)
    fig, ax = plt.subplots(figsize=(9, 6))
    colors = {
        "sybil": "#e11d48", "traffic_light": "#2563eb", "sensor_spoofing": "#16a34a", "universal_perturbation": "#8b5cf6", "fake_safety": "#f97316", "fake_emergency": "#06b6d4"
    }
    levels = ["L1", "L2", "L3"]
    ...
    for att, lvl_data in frontier_data.items():
        ...
        for idx, lvl in enumerate(levels):
            ...
            ax.scatter(lats[idx], tprs[idx], color=colors[att], s=sizes[idx], marker=markers[idx], edgecolor="black", linewidth=0.8, alpha=0.9, label=lbl)
            ax.annotate(lvl, (lats[idx], tprs[idx]), textcoords="offset points", xytext=(0, 6), ha="center", fontsize=8, fontweight="bold", color="#334155")
    ...
    if has_plotted_frontier:
        ...
        plt.title("Figure 10 (Luxembourg): The Detectability Frontier (TPR vs. Latency)\nTrajectory paths from L1 (Low) → L2 (Medium) → L3 (High) Intensity", fontsize=12, fontweight="bold", pad=15)
        plt.tight_layout()
        
        plt.savefig("/home/mehdi/VANET_Project/Docker_files/post_treatment/figure/luxembourg_runs_roc_pr_frontier.png", dpi=300)
        plt.savefig("/home/mehdi/VANET_Project/Docker_files/post_treatment/figure/luxembourg_runs_roc_pr_frontier.pdf", format="pdf", dpi=300)
        plt.savefig("/home/mehdi/VANET_Project/Docker_files/post_treatment/figure/luxembourg_frontier.pdf", format="pdf", dpi=300)
        print("\n✓ Saved Luxembourg Frontier visuals to luxembourg_runs_roc_pr_frontier.png/pdf & luxembourg_frontier.pdf")
