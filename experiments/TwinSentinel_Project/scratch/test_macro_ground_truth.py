...
def process_run(live_history, baseline_history):
    ...
    # Baseline compared to itself is always Saine (0)
    ...
    for w in range(num_windows):
        ...
        # Compare time-matched window
        # To avoid division by zero or tiny values, let's add a small epsilon if base speed is very low
        ...
        # For stopped ratio, since it is a ratio [0, 1], let's check absolute deviation <= 0.05 or relative +/-10%
    ...
    return distances, baseline_distances, y_live, y_baseline
...
for filepath in sorted(glob.glob(os.path.join(RUNS_DIR, "run_*.json"))):
    ...
    try:
        ...
        continue
    distances, baseline_distances, y_live, y_baseline = process_run(live_history, baseline_history)
    
    # Combine baseline and live
    all_distances = baseline_distances + distances
    y_true = y_baseline + y_live
    ...
    print(f"{filename}: AUC-ROC={auc_roc:.4f}, AUC-PR={auc_pr:.4f} (Positives: {num_pos}/{len(y_true)})")
