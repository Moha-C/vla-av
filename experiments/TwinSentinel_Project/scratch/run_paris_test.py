...
def run_test():
    ...
    # 3. Call launch_Paris
    print("Calling launch_Paris...")
    res = call_tool("launch_Paris", {}, headers)
    print("launch_Paris response:", res)
    if not res or "error" in res:
        return

    # 4. Wait for launch and connection
    print("Waiting 4 seconds for SUMO GUI to launch and connect...")
    time.sleep(4)

    # 5. Call start_simulation
    print("Calling start_simulation...")
    res = call_tool("start_simulation", {}, headers)
    print("start_simulation response:", res)

    # 6. Wait a bit and check stats
    print("Waiting 5 seconds for simulation progress...")
    time.sleep(5)
